...
namespace gpe
{
    animation2d_sdl::animation2d_sdl()
    {
        sprite_tex = NULL;
        ck_r = 255;
        ck_g = 0;
    ...
    animation2d_sdl::animation2d_sdl(  const std::string& anim_name, const std::string& anim_filename, bool transparent_image, uint8_t colorkeyR, uint8_t colorkeyG , uint8_t colorkeyB   )
    {
    ...
    bool animation2d_sdl::copy_image_source(std::string directory_output_name)
    {
        return animation2d::copy_image_source( directory_output_name);
    }

    animation2d * animation2d_sdl::copy_self()
    {
        return new animation2d_sdl( );
    }

    void animation2d_sdl::clean_up()
    {
        animation2d::clean_up();
    }

    animation2d * animation2d_sdl::create_new( const std::string& anim_name, const std::string& anim_filename, bool transparent_image, uint8_t colorkeyR, uint8_t colorkeyG , uint8_t colorkeyB )
    {
        return new animation2d_sdl( anim_name, anim_filename, transparent_image,  colorkeyR,  colorkeyG ,  colorkeyB );
    }

    void animation2d_sdl::load_image(  const std::string & anim_filename, bool transparent, uint8_t colorkeyR, uint8_t colorkeyG, uint8_t colorkeyB  )
    {
        clean_up();
	...
	bool animation2d_sdl::render_quad( int sub_image_to_draw, shape_point2d p1 , shape_point2d p2, shape_point2d p3, shape_point2d p4, color * render_color, int alpha , shape_rect * cam   )
	{
    	...
    	return ( SDL_RenderGeometry( renderer_main_sdl->get_sdl_renderer(), sprite_tex->get_sdl_texture(), vert, 4, new_indices, 6 ) == 0 );
    }

    void animation2d_sdl::setup_animation( int frame_count, int aw, int ah, int sofx, int sofy, int hPad, int vPad )
    {
        animation2d::setup_animation( frame_count, aw, ah, sofx, sofy, hPad, vPad );
        setup_quad_coordinates();
    }

    void animation2d_sdl::setup_fullimg_animation(  int aw, int ah, int sofx, int sofy, int hPad, int vPad )
    {
    ...
    void animation2d_sdl::setup_quad_coordinates()
    {
        sprite_quad_tex_top_left.clear();
        sprite_quad_tex_top_right.clear();
        sprite_quad_tex_bottom_left.clear();
