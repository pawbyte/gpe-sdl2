#ifndef GPE_ANIMIATION2D_SDL_H
#define GPE_ANIMIATION2D_SDL_H
...
namespace gpe
{
    ...
    class animation2d_sdl: public animation2d
    {
        protected:
            /*
        ...
        private:
            texture_sdl * sprite_tex; //we will store the actual SDL texture of the sprite to avoid typecasting for our quad function and "if later added" other functions
        public:
            animation2d_sdl();
            animation2d_sdl(  const std::string &anim_name, const std::string& anim_filename, bool transparent_image = false, uint8_t colorkeyR = 255, uint8_t colorkeyG = 0, uint8_t colorkeyB = 255);
            virtual ~animation2d_sdl();
